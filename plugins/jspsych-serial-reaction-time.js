...
jsPsych.plugins["serial-reaction-time"] = (function() {
  ...
  plugin.trial = function(display_element, trial) {
	...
	trial.boxcolor = trial.boxcolor || '#999999';
    trial.response_ends_trial = (typeof trial.response_ends_trial === 'undefined') ? true : trial.response_ends_trial;
    trial.timing_pre_target = (typeof trial.timing_pre_target === 'undefined') ? 0 : trial.timing_pre_target;
    trial.timing_max_duration = trial.timing_max_duration || 1000; // if -1, then wait for response forever
    ...
    if (trial.prompt !== "") {
      ...
      if(trial.fade_duration == -1){
        $('#jspsych-serial-reaction-time-stimulus-cell-'+trial.target[0]+'-'+trial.target[1]).css('backgroundColor',trial.boxcolor[trial.target[0]][trial.target[1]]);
      } else {
        $('#jspsych-serial-reaction-time-stimulus-cell-'+trial.target[0]+'-'+trial.target[1]).animate({
			'background-color':trial.boxcolor[trial.target[0]][trial.target[1]]
        }, trial.fade_duration);
      }
